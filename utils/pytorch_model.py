...
class BankingNet(nn.Module):
    ...
    Architecture identique au modèle RegressionNet du notebook.
    """
    ...
    def __init__(self, input_size=9):
        ...
        Args:
            input_size (int): Nombre de features d'entrée (9 par défaut)
        """
        super(BankingNet, self).__init__()
        
        # Couches du réseau (architecture exacte du notebook)
    ...
    def forward(self, x):
        ...
        # dropout1 commenté comme dans le notebook
        # x = self.dropout1(x)
        x = self.relu(self.layer2(x))
        # dropout2 commenté comme dans le notebook
        # x = self.dropout2(x)
